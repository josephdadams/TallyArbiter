...
1. Now that all the necessary libraries are installed and compiled, you need to copy the `gpo-listener.py` file to your Pi (and the config example file `config_gpo.json.example`). You can do this a number of ways, but one simple way is to execute this command through your SSH connection: `wget https://github.com/josephdadams/TallyArbiter/raw/HEAD/listener_clients/gpo-listener/gpo-listener.py; wget https://github.com/josephdadams/TallyArbiter/raw/HEAD/listener_clients/gpo-listener/config_gpo.json.example`. This will copy the file into your current folder (you should still be the home folder for the `pi` account).
