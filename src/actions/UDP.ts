...
@RegisterAction("79e3ce28", "Generic UDP", [
    ...
    { fieldName: 'type', fieldLabel: 'UDP socket family (interface type)', fieldType: 'dropdown', options: [{ id: 'udp4', label: 'IPv4' }, { id: 'udp6', label: 'IPv6' }], optional: true }
])
    ...
    public run(): void {
        try {
            ...
            client.send(sendBuf, this.action.data.port, this.action.data.ip, function(error) {
                if (!error) {
                    logger(`Generic UDP sent: ${this.action.data.ip}:${this.action.data.port} : ${this.action.data.string}`, 'info');
                }
                client.close();
            });
        } catch (error) {
