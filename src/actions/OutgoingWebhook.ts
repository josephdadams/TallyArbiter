...
export class OutgoingWebhook extends Action {
    public run(): void {
        try {
            let path = this.action.data.path ? (this.action.data.path.startsWith('/') ? this.action.data.path : '/' + this.action.data.path) : '';
            this.action.data.protocol = this.action.data.protocol || 'http://';
    
            this.action.data.port = this.action.data.port ? (this.action.data.port === '' ? '80' : this.action.data.port) : '80'; //explicitly set the port to 80 if they did not specify
