	...
	properties: {
		...
		cloud_destinations: {
			...
			items: {
				...
				properties: {
					...
					connected: {
						description: 'Whether the cloud destination is connected',
						type: 'boolean',
					},
				},
				required: ['host', 'port', 'key', 'id', 'status', 'connected'],
			},
			uniqueItems: true,
		},
		cloud_keys: {
		...
		device_actions: {
			...
			items: {
				...
				required: ['deviceId', 'data', 'busId', 'active', 'outputTypeId', 'id'],
			},
			uniqueItems: true,
		},
		device_sources: {
		...
		sources: {
			...
			items: {
				...
				required: ['id', 'name', 'sourceTypeId', 'enabled', 'reconnect', 'connected', 'reconnect_interval', 'max_reconnects'],
			},
			uniqueItems: true,
		},
		tsl_clients: {
		...
		bus_options: {
			...
			items: {
				...
				properties: {
					...
					visible: {
						description: 'Bus visibility',
						type: 'boolean',
					},
				},
				required: ['id', 'label', 'type', 'color', 'priority', 'visible'],
			},
			minItems: 1,
			uniqueItems: true,
		},
